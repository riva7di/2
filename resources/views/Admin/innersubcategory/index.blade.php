                ...
                <div class="col-12">
                    ...
                    <div class="card">                       
                        <div class="card-header">
                            <h4 class="card-title">{{ trans('labels.innersubcategory') }}</h4>
                            <a href="{{route('admin.innersubcategory.add')}}" class="btn btn-raised btn-primary btn-min-width mr-1 mb-1 float-right" style="margin-top: -30px;">
                                {{ trans('labels.add_innersubcategory') }}
                            </a>
                        </div>

                        <div class="col-md-4">
                            <form method="GET" action="{{route('admin.innersubcategory.search')}}">
                        ...
                        <div class="card-body collapse show">
                            <div class="card-block card-dashboard" id="table-display">
                                    @include('Admin.innersubcategory.innersubcategorytable')
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    function CategoryTable() {
        $('#preloader').show();
        $.ajax({
            url:"{{ route('admin.innersubcategory.list') }}",
            method:'get',
        ...
        Swal.fire({
            ...
            if(t.value==true){
                ...
                $.ajax({
                    ...
                    url: '{{route("admin.innersubcategory.changeStatus")}}',
                    type: "POST",
                    ...
                    { 
                        ...
                        if(data == 1000)
                        {
                            Swal.fire({type: 'success',title: '{{ trans('labels.success') }}',text: "Change status successfully",showConfirmButton: false,timer: 1500});    
                            if(status=='1'){
